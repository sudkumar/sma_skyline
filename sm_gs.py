...
class GSA():
	...
	def __init__(self, setA, setB, aQ, aR):
		self.a = setA
		self.b = setB
		self.q = aQ
		self.r = aR
	...
	def computePreferenceList1(self, person, opposite_list):
		...
		for i in range(0,len(opposite_list)):
			(id, score) = (opposite_list[i][0], sum([a*b for a,b in zip(person[2], opposite_list[i][1])]))
	...
	def computePreferenceList2(self, person, opposite_list):
		score_list = []
		for i in range(0,len(opposite_list)):
			score = sum([a*b for a,b in zip(person[2], opposite_list[i][1])])
	...
	def Marriage(self):
		...
		partner_list = [(item[0],-1) for item in listB]
...
def main():
	...
	afp = open("aData.txt", "r")
	setA = []
	for line  in afp:
		splitedLine = line.split()
		id = int(splitedLine[0])
		qualities = map(int, splitedLine[1:aQDims+1])
		requires = map(float,splitedLine[aQDims+1:])
		setA.append([id, qualities, requires])
	afp.close()

	bfp = open("bData.txt", "r")
	setB = []
	for line  in bfp:
		splitedLine = line.split()
		id = int(splitedLine[0])
		qualities = map(int, splitedLine[1:aRDims+1])
		requires = map(float,splitedLine[aRDims+1:])
		setB.append([id, qualities, requires])
	bfp.close()


	gsa = GSA(setA, setB, aQDims, aRDims)
